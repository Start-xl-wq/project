...
Wakeup 逻辑独立于 USB clock，通过检测 DP/DM 电平或线状态变化产生 wakeup 中断，usb reset后该中断不能触发。
...
### 场景 3：Host 被其他源唤醒，从系统 Suspend 恢复

1. Host 进入系统 suspend.
2. USB clock 关闭，USB controller被 reset.
3. 原有 controller 配置和链路上下文丢失.
4. Host 被其他唤醒源唤醒并执行系统 resume.
5. 重新初始化 USB controller.
6. 根据当前端口状态重新建立连接.
7. 重新枚举 device。

### 场景 4：Host 被usb device唤醒，从系统 Suspend 恢复

1. Host 进入系统 suspend.
2. USB clock 关闭，USB controller被 reset.
3. 原有 controller 配置和链路上下文丢失.
4. Device 主动拉 K，由于host 进行了usb reset导致wakeup失效，所以不能被唤醒.
...
5. 由于host 进行了usb reset导致wakeup失效.
6. 所以对device的插拔均无感知，且不会被唤醒。